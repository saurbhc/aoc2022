...
def compute(s: str) -> int:
    ...
    for line in lines:
        ...
        if is_same_choice(p1, p2):
            # draw
            score = get_choice_score(p2) + get_draw_score()
            # print(f'draw {p1=} {p2=} {score=}')
            my_points += score
        elif is_rock(p2):
            if is_scissors_p1(p1):
                # win
                score = get_choice_score(p2) + get_win_score()
                # print(f'win {p1=} {p2=} {score=}')
                my_points += score
            else:
                # lose
                score = get_choice_score(p2) + get_lost_score()
